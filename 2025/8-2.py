...
last_pair_indexes = None
num_connections = 0
for dist, i, j in pairs:
    ci, cj = get_last_connection(i, last_connection), get_last_connection(j, last_connection)
    if ci != cj:
        last_connection[cj] = ci
        last_pair_indexes = (i, j)
        num_connections += 1
        if num_connections == num_junction_boxes - 1:
            break
print(f"last_pair_indexes: {last_pair_indexes}")

x1, y1, z1 = coords[last_pair_indexes[0]]
x2, y2, z2 = coords[last_pair_indexes[1]]
result = x1 * x2

print(f"result: {result}")
# Correct answers: 25272 and 22517595
